...
export default function CloseWeekWizard({ snap, orders, origId, onClose, onConfirm }: Props) {
      ...
      for (const mc of (row.machineCells ?? [])) {
        for (const w of (mc.work ?? [])) {
          ...
          if (w.order_id) {
            ...
            doneQty = o?.done_qty ?? 0
          } else {
            doneQty = orders.find(x => x.sap_so === w.sap_so)?.done_qty ?? 0
          }
  ...
  const machineStats = useMemo(() => {
    ...
    for (const row of (fullSnap.plan_data?.dayRows ?? [])) {
      for (const mc of (row.machineCells ?? [])) {
        ...
        stats[mc.machineId].wall += mc.wall ?? 0
        stats[mc.machineId].capH += mc.capH ?? 0
      }
    }
  ...
  const modal: React.CSSProperties = { background: 'var(--bg)', border: '1px solid var(--bord)', borderRadius: 14, width: 760, maxWidth: '96vw', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px' }}>
          ...
          {!done && step === 1 && (<>
            ...
            <table style={tbl}><thead><tr>
              {['SAP SO','ลูกค้า','kVA','แผน','เสร็จ','ค้าง','Planned Finish','สถานะ'].map(h => <th key={h} style={th}>{h}</th>)}
            </tr></thead><tbody>
              {orderList.map(o => (
                <tr key={o.key} style={{ background: o.status === 'completed' ? 'rgba(166,227,161,.04)' : o.status === 'partial' ? 'rgba(249,226,175,.04)' : 'rgba(224,90,78,.04)' }}>
                  ...
                  <td style={{ ...td, color: 'var(--txt2)', maxWidth: 90, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{o.customer}</td>
              ...
              {!orderList.length && <tr><td colSpan={8} style={{ ...td, textAlign: 'center' as const, color: 'var(--txt3)', padding: 28 }}>ไม่พบ orders ใน snapshot นี้</td></tr>}
            </tbody></table>
          </>)}
          ...
          {done && result && (() => {
            ...
            return (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '20px 0' }}>
                ...
                <div style={{ background: 'var(--bg2)', borderRadius: 12, border: `1px solid ${col}44`, padding: '20px 32px', textAlign: 'center' as const, minWidth: 300 }}>
                  ...
                  {(result.best_machine || result.bottleneck_machine) && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 14, fontSize: 10 }}>
